...
def plot_phase_chirp():
    ...
    plt.legend()
    plt.title(f"chirp signal in frequency range [{fmin/1000}, {fmax/1000}] KHz, period: {period} sec")
    plt.ylabel("instant frequency")
    plt.grid(True)

    plt.subplot(212)
    plt.plot(time_vec, phase_inst, linewidth=2, label=r"$\phi_{inst}(t)$")
    plt.plot(time_vec, phase_est, "-.", linewidth=2, label=r"estimated $\phi_{est}(t)$ using HT")
