# ----------------------------------------------------------------------------------------------------------------------
# This module investigates the localization performance for the conventional beamforming when there are several targets.
#
#
...
def beamformer_multiple_targets_sin():
    ...
    for freq_design in tqdm(freq_design_vec):
        ...
        filename = os.path.join(
            root, f"multiple_targets_beamformer_sin_freq={freq_design}.pdf"
        )
        ...
        # apply beamforming to the input signal
        sig_bf = beamf.apply_to_signal(bf_mat=bf_mat, sig_in=sig_in)
...
def beamformer_multiple_targets_wideband():
    """
    this function evaluates the performance of conventional localization for the wideband scenario.
    """
    # find the directory for this file
    root = os.path.join(
        Path(__file__).resolve().parent, "beamformer_multiple_targets_wideband"
    )
    ...
    for center_freq in tqdm(center_freq_vec):
        ...
        # compute the power along different DoAs in the grid
        power_bf = np.mean(sig_bf ** 2, axis=0)
        ...
        filename = os.path.join(
            root, f"multiple_targets_beamformer_wideband_center_freq={center_freq}.pdf"
        )

        plot_beampattern(doa_list, power_bf, f"$F_c= {center_freq / 1000:0.0f}$ kHz, $B={bandwidth / 1000:0.0f}$ kHz", filename)
...
def main():
    beamformer_multiple_targets_sin()
    # beamformer_multiple_targets_wideband()
