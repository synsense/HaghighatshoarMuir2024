...
def array_resolution_sin():
    ...
    root = os.path.join(Path(__file__).resolve().parent, "array_resolution_snn_sin")
    ...
    for freq_design in tqdm(freq_design_vec):
        ...
        beamf = SNNBeamformer(geometry=geometry, kernel_duration=kernel_duration, freq_range=freq_range,
                              tau_vec=tau_vec, fs=fs)

        print(f"\n\nplotting beam pattern for freq: {freq_design}...")

        filename = os.path.join(root, f"array_resolution_snn_sin_freq={freq_design}.pgf")
...
def array_resolution_wideband():
    ...
    """
    # find the directory for this file
    root = os.path.join(Path(__file__).resolve().parent, "array_resolution_snn_wideband")
    ...
    for center_freq in tqdm(center_freq_vec):
        ...
        filename = os.path.join(root, f"array_resolution_snn_wideband_fc={center_freq}.pgf")
