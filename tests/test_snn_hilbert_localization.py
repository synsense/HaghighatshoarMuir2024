...
def test_array_resolution():
    ...
    measure_noise = (
        1 / np.sqrt(np.mean(sig_temp**2)) * np.random.randn(len(time_temp))
    )
    ...
    freq_inst = freq_min + (freq_max - freq_min) * time_temp / time_temp[-1]
    ...
    sig_bf = beamf.apply_to_template(
        bf_mat=bf_mat,
        template=(time_temp, sig_temp, doa_target * np.ones(len(sig_temp))),
    ...
    plt.title(
        f"array resolution: freq={freq_design}, ker-duration:{kernel_duration} sec"
    )
    plt.grid(True)
    ...
    plt.axvline(
        x=doa_target * 180 / np.pi,
        label=f"target DoA={doa_target * 180 / np.pi:0.1f}",
        color="r",
    )
    plt.legend()
